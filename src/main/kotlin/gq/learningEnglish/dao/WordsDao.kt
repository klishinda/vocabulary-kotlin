...
class WordsDao(private val jdbc: JdbcDao) {
    ...
    fun addWord(word: Word, userId: Long): Long = jdbc.namedQuery(ADD_WORD, word.sqlParams.addValue("userId", userId))
}

private const val ADD_WORD =
    """insert into words(id, user_id, word, language, description, part_of_speech)
    values (default, :userId, upper(:word), :language, :description, :part_of_speech) returning id"""
